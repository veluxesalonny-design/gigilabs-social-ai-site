...
import { baseRpc as rpc, latestBaseBlock, sanitizeRpcError } from './rpc.js';
...
const decimalsCache=new Map(),poolCache=new Map(),routerFactoryCache=new Map(),exactCache=new Map();
let flashCache={at:0,value:null};
function exactKey(o,block){return [block,o.assetAddress,o.quoteAddress,o.buyDex,o.buyPair,o.sellDex,o.sellPair,Number(o.optimalBorrow||o.borrowUsd||o.b)].map(norm).join('|')}
function pruneExact(){if(exactCache.size<300)return;const now=Date.now();for(const[k,v]of exactCache)if(now-v.at>30000)exactCache.delete(k);while(exactCache.size>250)exactCache.delete(exactCache.keys().next().value)}
...
async function poolMeta(block,dex,pair,tin,tout){
  const key=[dex,norm(pair),norm(tin),norm(tout)].join(':'),cached=poolCache.get(key);if(cached&&Date.now()-cached.at<3600000)return cached.value;
  const t0=await call(block,pair,pairBase,'token0');
  const t1=await call(block,pair,pairBase,'token1');
  const factory=await call(block,pair,pairBase,'factory');
  if(!tokensMatch(t0,t1,tin,tout))throw Error(dex+' token mismatch');
  let meta;
  if(dex==='Uniswap'){
    ...
    if(norm(factory)===norm(uv2))meta={kind:'V2',venue:0,pair,factory,t0,t1,fee:0,tickSpacing:0,stable:false};
      ...
      const fee=Number(await call(block,pair,v3Pool,'fee'));if(![100,500,3000,10000].includes(fee))throw Error('unsupported Uniswap fee');
      const canonical=await call(block,UNI_V3_FACTORY,v3Factory,'getPool',[tin,tout,fee]);if(norm(canonical)!==norm(pair))throw Error('Uniswap V3 pool mismatch');
      meta={kind:'V3',venue:0,pair,factory,t0,t1,fee,tickSpacing:0,stable:false};
    }else throw Error('Uniswap factory unsupported');
  }else if(dex==='Sushi'){
    const sf=await routerFactory(SUSHI_V2_ROUTER,block);if(norm(factory)!==norm(sf))throw Error('Sushi factory mismatch');
    meta={kind:'V2',venue:1,pair,factory,t0,t1,fee:0,tickSpacing:0,stable:false};
  }else if(dex==='Aerodrome'){
    if(norm(factory)===norm(AERO_CLASSIC_FACTORY)){
      const stable=Boolean(await call(block,pair,aeroClassic,'stable'));meta={kind:'AERO_V2',venue:0,pair,factory,t0,t1,fee:0,tickSpacing:0,stable};
    }else if(norm(factory)===norm(AERO_CL_FACTORY)){
      const tickSpacing=Number(await call(block,pair,aeroClPool,'tickSpacing'));if(!Number.isFinite(tickSpacing)||tickSpacing<=0)throw Error('invalid Aerodrome tick spacing');
      const canonical=await call(block,AERO_CL_FACTORY,aeroClFactory,'getPool',[tin,tout,tickSpacing]);if(norm(canonical)!==norm(pair))throw Error('Aerodrome CL pool mismatch');
      meta={kind:'AERO_CL',venue:0,pair,factory,t0,t1,fee:0,tickSpacing,stable:false};
...
async function quoteMeta(block,meta,tin,tout,amountIn){
  ...
  }else if(meta.kind==='V3'){
    const q=await call(block,UNI_V3_QUOTER,v3Quoter,'quoteExactInputSingle',[{tokenIn:tin,tokenOut:tout,amountIn,fee:meta.fee,sqrtPriceLimitX96:0n}]);out=BigInt(q[0]);
  }else if(meta.kind==='AERO_V2')out=BigInt(await call(block,meta.pair,aeroClassic,'getAmountOut',[amountIn,tin]));
  else if(meta.kind==='AERO_CL'){
    const q=await call(block,AERO_CL_QUOTER,aeroClQuoter,'quoteExactInputSingle',[{tokenIn:tin,tokenOut:tout,amountIn,tickSpacing:meta.tickSpacing,sqrtPriceLimitX96:0n}]);out=BigInt(q[0]);
  }
...
export async function exactBase(o){
  let phase='HEAD';
  try{
    const block=await latestBaseBlock(),key=exactKey(o,block),cached=exactCache.get(key);if(cached&&Date.now()-cached.at<20000)return cached.value;
    phase='META';
