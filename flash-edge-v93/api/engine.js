...
async function fetchScreenPairs(){
  ...
  const pairs=[...dedup.values()];screenCache={at:Date.now(),pairs};return pairs;
}
...
function buildScreenRoutes(pairs,cap){
  const routes=[];for(const asset of ASSETS){const pools=pairs.map(p=>orientPair(p,asset)).filter(Boolean);for(const buy of pools)for(const sell of pools){if(norm(buy.pair)===norm(sell.pair)||sell.px<=buy.px)continue;const spreadBps=(sell.px/buy.px-1)*10000;if(spreadBps<3)continue;const liq=Math.min(buy.liq,sell.liq),max=Math.min(250000,cap,Math.max(0,liq*.01));if(max<1000)continue;const ladder=[...new Set([...MASTER_SIZES.filter(x=>x<=max),Math.round(max/100)*100].filter(x=>x>=1000&&x<=max))].sort((a,b)=>a-b);if(!ladder.length)continue;const scored=ladder.map(size=>({size,...modelNet(buy.px,sell.px,size,liq)})).sort((a,b)=>b.net-a.net),best=scored[0];if(!best||best.net<-12)continue;routes.push({id:[asset.symbol,norm(buy.pair),norm(sell.pair)].join(':'),k:'base',chain:'Base',chainId:8453,assetSymbol:asset.symbol,assetAddress:asset.address,quoteSymbol:'USDC',quoteAddress:USDC,buyDex:buy.dex,sellDex:sell.dex,bd:buy.dex,sd:sell.dex,buyPair:buy.pair,sellPair:sell.pair,sameDex:buy.dex===sell.dex,spreadBps,liquidityUsd:liq,maxBorrowUsd:max,borrowUsd:best.size,b:best.size,optimalBorrow:best.size,screenNet:best.net,screenGross:best.gross,sizeCandidates:scored.slice(0,3).map(x=>x.size),testedSizes:scored.map(x=>({borrowUsd:x.size,screenNet:x.net}))})}}
...
async function exactRoute(route,budget){const sizes=(route.sizeCandidates||[route.borrowUsd]).slice(0,budget),tests=[];for(const size of sizes){const env=await exactBase({...route,optimalBorrow:size,borrowUsd:size,b:size,gas:.75});tests.push({...env,_size:size})}const ranked=tests.slice().sort((a,b)=>(b.exact?.netAfterGas??-Infinity)-(a.exact?.netAfterGas??-Infinity)),best=ranked.find(x=>x.stage==='EXACT_PASS'&&x.exact?.status==='PASS')||ranked[0]||{stage:'UNAVAILABLE',exact:{status:'BLOCKED',reason:'no exact result'}};return{...best,proofSource:'server',selectedBorrowUsd:best._size,exactAt:best.exactAt||Date.now(),testedSizes:tests.map(x=>({borrowUsd:x._size,stage:x.stage,netAfterGas:x.exact?.netAfterGas??null}))}}
async function scan(cap){
  const started=Date.now(),blockNumber=await latestBlock(),pairs=await fetchScreenPairs(),allRoutes=buildScreenRoutes(pairs,cap),searchRoutes=allRoutes.slice(0,48),exactById=new Map(),blockedPairs=new Set();let exactChecks=0,exactRoutes=0;
  for(const r of searchRoutes){
    if(exactChecks>=10||exactRoutes>=5)break;const bk=norm(r.buyPair),sk=norm(r.sellPair);if(blockedPairs.has(bk)||blockedPairs.has(sk))continue;
  ...
  return{ok:true,engine:'Flash Edge V9.4.2',mode:'deep broad screen → unsupported-pool suppression → bounded exact → live prepare',rpcPolicy:RPC_LABEL,blockNumber,latencyMs:Date.now()-started,assets:ASSETS.length,venues:3,searchCapUsd:cap,screenPairs:pairs.length,screenRouteCount:allRoutes.length,searchDepth:searchRoutes.length,candidateCount:candidates.length,sameDexCandidates:candidates.filter(c=>c.sameDex).length,exactRoutes,exactChecks,suppressedUnsupported,exactPasses,candidates};
}
async function receiverCheck(receiver,owner){if(!validAddress(receiver)||!validAddress(owner))throw Error('invalid receiver or owner');const code=await rpc('eth_getCode',[receiver,'latest']);if(!code||code==='0x')throw Error('no receiver bytecode');const hash='0x'+createHash('sha256').update(String(code).toLowerCase()).digest('hex');if(norm(hash)!==norm(APPROVED_RUNTIME))throw Error('receiver runtime mismatch');async function read(fn){const data=encodeFunctionData({abi:RX,functionName:fn}),out=await call(receiver,data,'latest');return decodeFunctionResult({abi:RX,functionName:fn,data:out})}const[chainOwner,paused,cap]=await Promise.all([read('owner'),read('paused'),read('maxBorrowRaw')]);if(norm(chainOwner)!==norm(owner))throw Error('owner mismatch');return{ok:true,owner:chainOwner,paused:Boolean(paused),capUsd:Number(cap)/1e6,runtimeHash:hash}}
export default async function handler(req,res){try{const action=String(req.query?.action||req.body?.action||'health');if(action==='health')return ok(res,{ok:true,engine:'Flash Edge V9.4.2',mode:'deep broad screen → unsupported-pool suppression → bounded exact → live prepare',rpcPolicy:RPC_LABEL,assets:ASSETS.length,maxSearchUsd:250000});if(action==='block')return ok(res,{ok:true,blockNumber:await latestBlock(),serverTime:Date.now(),rpcPolicy:RPC_LABEL});if(action==='scan')return ok(res,await scan(Math.min(250000,Math.max(1000,Number(req.body?.cap)||250000))));if(action==='receiver')return ok(res,await receiverCheck(String(req.body?.receiver||''),String(req.body?.owner||'')));if(action==='prepare'){const c=req.body?.candidate,env=c?.exactEnvelope;if(env?.stage!=='EXACT_PASS'||env?.exact?.status!=='PASS')return fail(res,'fresh EXACT_PASS required',409);const route={...c,chainKey:'base',optimalBorrow:Number(env.selectedBorrowUsd||c.borrowUsd||c.b)};return ok(res,await prepareLive({route,receiver:String(req.body?.receiver||''),from:String(req.body?.from||'')}))}if(action==='rpc'){const method=String(req.body?.method||''),allowed=new Set(['eth_getBalance','eth_getTransactionCount','eth_estimateGas','eth_gasPrice','eth_sendRawTransaction','eth_getTransactionReceipt','eth_chainId']);if(!allowed.has(method))return fail(res,'RPC method blocked',403);return ok(res,{ok:true,result:await rpc(method,Array.isArray(req.body?.params)?req.body.params:[])})}return fail(res,'unknown action',404)}catch(e){console.error('v942-engine',e);return fail(res,e?.message||String(e),503)}}
